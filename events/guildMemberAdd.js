...
module.exports = {
  ...
  async execute(member, client) {
    try {
      // 1. Give Role Immediately
      if (config.autoRoleId) {
        const role = member.guild.roles.cache.get(config.autoRoleId);
        if (role) {
          member.roles.add(role).catch(err => {
            console.error(`[AUTO-ROLE-ERROR] Failed to add role to ${member.user.tag}:`, err.message);
          });
        }
      }

      let gateChannel = member.guild.channels.cache.get(config.gatewayChannelId);
      
      if (!gateChannel && config.gatewayChannelId) {
        gateChannel = await member.guild.channels.fetch(config.gatewayChannelId).catch(() => null);
      }

      if (!gateChannel) {
        gateChannel = member.guild.channels.cache.find(c => c.name.toLowerCase().includes("gateway") || c.name.toLowerCase().includes("gateaway"));
      }
      ...
      if (gateChannel) {
        ...
        try {
          ...
          const bannerUrl = botUser.bannerURL({ size: 1024, extension: 'png' }) || botUser.avatarURL({ size: 1024, extension: 'png' });
        ...
        if (cardBuffer) {
          msgData.files = [new AttachmentBuilder(cardBuffer, { name: 'welcome.png' })];
        }

        await gateChannel.send(msgData).catch(() => {});
      }

      // Admin Log keep alive
      advanceLog(client, {
        type: "success",
        title: "Member Joined",
        activity: "Gateway",
