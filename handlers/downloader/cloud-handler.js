...
async function runCloudFlow(target, url) {
    ...
    const platform = url.includes("mediafire.com") ? "Mediafire" : url.includes("drive.google.com") ? "GDrive" : "MEGA";
    const initialEmbed = getStatusEmbed(`${platform} Storage Hub`, "Secure link check in progress...");
    ...
    try {
        ...
        if (platform === "Mediafire") {
            ...
            directUrl = $("a#downloadButton").attr("href") || $("a.input.popsok").attr("href");
            title = $(".dl-btn-label").text().trim() || $(".filename").text().trim() || "MF File";
        } else if (platform === "GDrive") {
            const fileIdMatch = url.match(/(?:\/d\/|id=|uc\?id=)([a-zA-Z0-9_-]{10,})/);
            ...
            directUrl = url; // MEGA handles its own stream
        }
        ...
        db.jobs[jobId] = {
            ...
            timestamp: Date.now(),
            title: title,
            stats: { likes: "---", views: "---", comments: "---", shares: "---", duration: size, uploader: platform },
            thumbnail: "",
            platform: platform,
        ...
        await editResponse({
            embeds: [getStatusEmbed("Cloud Hub Extraction Failed", e.message || "Access blocked or link dead.")]
        });
    }
}
